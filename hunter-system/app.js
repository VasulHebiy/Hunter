...
(function () {
  ...
  const SPECS = [
    ...
    { id:"elemental",   name:"Маг",        param:"Стихійна сила",         keys:[["int",0.6],["per",0.3],["wil",0.1]] },
    { id:"warlock",     name:"Чаклун",     param:"Сила заклять",          keys:[["int",0.6],["wil",0.3],["per",0.1]] },
    { id:"illusionist", name:"Фокусник",   param:"Ілюзійна майстерність", keys:[["agi",0.6],["int",0.3],["per",0.1]] },
    { id:"witcher",     name:"Відьмак",    param:"Знаки",                 keys:[["str",0.6],["int",0.3],["wil",0.1]] },
    { id:"guardian",    name:"Страж",      param:"Стійкість",             keys:[["sta",0.6],["wil",0.3],["str",0.1]] },
    { id:"seer",        name:"Провидець",  param:"Передбачення",          keys:[["per",0.6],["int",0.3],["wil",0.1]] },
    { id:"manipulator", name:"Маніпулятор",param:"Вплив",                 keys:[["wil",0.6],["int",0.3],["per",0.1]] },
    { id:"raider",      name:"Рейдер",     param:"Мобільність",           keys:[["agi",0.6],["rea",0.3],["sta",0.1]] },
  ];
  ...
  const CULTS = [
    ...
    },
    {
      ...
      onJoin(h){
        // +5..+8 to each stat (not under risk check)
        STATS.forEach(s=>{
          const add = randInt(5,8);
          h.stats[s.key] = (Number(h.stats[s.key])||0) + add;
        });
        // Add trait "Бездушний" in traits block
        addTrait(h, "soulless");
      }
    },
    {
    ...
    },
    {
      ...
      describe(rank){
        const secs = [2,4,6,8,10,12][rank] || 2;
        const lines = [
          "Тіньовий хід",
          `Вхід у тінь: ${secs} с`,
          "Коротка невидимість у тіні",
        ];
  ...
  function joinCult(h, cultId){
    ...
    if (cult.onJoin) cult.onJoin(h);
    recomputeHunter(h);
    return {ok:true, msg:`Вступив: ${cult.name}`};
  }
        ...
        item.innerHTML = `
          ...
          <div class="item__body" role="region" aria-label="Деталі ханта">
            ...
            <div class="note" style="margin-top:10px">
              ...
              <div class="actions" style="justify-content:flex-start; margin-top:10px; gap:8px; flex-wrap:wrap">
                <button class="btn btn--primary" type="button" data-traitgain="${escapeHtml(h.id)}">Отримати (рандом)</button>
    ...
    function render(){
      ...
      CULTS.forEach(c=>{
        ...
        card.querySelector("[data-join]")?.addEventListener("click", ()=>{
          ...
          if (!confirm(`Вступити в культ "${c.name}"? Це незворотно.`)) return;
          const res = joinCult(hh, c.id);
          // BAAL join should NOT trigger risk (we don't clamp here)
          saveHunters(hs);
          toast(res.msg);
          render();
        });
