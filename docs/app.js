        ...
        <div class="note" style="margin-top:10px">
          ...
          <div class="skills">
            ${group.map(sk=>{
              ...
              const intMult = intManaMult(h?.stats?.int);
              ...
              const costTxt = sk.type==="mana"
                ? `Мана: ${Math.max(0, Math.round(sk.mana * tierMult * intMult))}`
                : "Без мани";
...
function getUnlockedSkills(h){
  ...
  const intMult = intManaMult(h?.stats?.int); // -0.75% per INT
  return unlocked.map(sk=>{
    ...
    if (out.type === "mana" && typeof out.mana === "number"){
      out.mana = Math.max(0, Math.round(out.mana * mult * intMult));
    }
    out._tierMult = mult;
    return out;
  });
  ...
  function initCultsPage(){
    ...
    function render(){
      ...
      CULTS.forEach(c=>{
        ...
        const desc = baseArr.slice(0,3).join(" • ");
        card.innerHTML = `
          <button class="item__head" aria-expanded="false">
            <div>
